...
bot_token = os.environ["5146303760:AAHxQ4iKiAoc-_hSA8y3YPsKuBr8bYJH0-Q"]
api_id = os.environ["14681595"]
...
async def process_file(file,bot,ev,msg):
    #try:
        ...
        txtfile.write(msgurls)
        txtfile.close()
        await bot.send_file(ev.message.chat,str(file)+'.txt')
        txtfile.close()
...
def init():
    try:
        ...
        async def process(ev: events.NewMessage.Event):
                ...
                if text == '/t':
                    moodle = MoodleClient(conf.moodleUser,conf.moodlepassword)
                    print(conf.moodleUser,conf.moodlepassword)
                    return
                    l = moodle.login()
                    print(l)
                    return
